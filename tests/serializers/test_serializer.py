...
def test_validate_field_error():
    ...
    assert e.value.errors == {"foo": ["Uh oh!"]}
...
def test_field_error():
    ...
    assert e.value.errors == {"foo": ["A valid integer is required."]}
...
def test_nested_error():
    ...
    assert e.value.errors == {"foo": {"foo": ["A valid integer is required."]}}
...
def test_is_valid():
    ...
    assert serializer.errors == {"foo": ["A valid integer is required."]}
...
def test_context():
    ...
    assert serializer.data == {"foo": {"foo": "Hello!"}}
...
@pytest.mark.parametrize("data", [{}, [], True, False, 123, 123.456])
