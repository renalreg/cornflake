...
@pytest.mark.parametrize(
    ...
    [
        ([], []),
        (
            [{"foo": date(2016, 1, 1)}, {"foo": date(2016, 1, 2)}],
            [{"foo": "2016-01-01"}, {"foo": "2016-01-02"}],
        ),
        (
            [{"foo": date(2016, 1, 1)}, None, {"foo": date(2016, 1, 2)}],
            [{"foo": "2016-01-01"}, None, {"foo": "2016-01-02"}],
        ),
    ],
)
...
def test_error():
    ...
    assert serializer.errors == {1: {"foo": ["Invalid date format."]}}
