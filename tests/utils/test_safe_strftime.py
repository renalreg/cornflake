...
def test_iso_format():
    assert (
        safe_strftime(
            datetime(1234, 1, 2, 3, 4, 5, tzinfo=pytz.utc), "%Y-%m-%dT%H:%M:%SZ"
        )
...
def test_yyyymmdd():
    assert (
        safe_strftime(datetime(1234, 1, 2, 3, 4, 5, tzinfo=pytz.utc), "%Y-%m-%d")
        == "1234-01-02"
    )
...
def test_ddmmyyyy():
    assert (
        safe_strftime(datetime(1234, 1, 2, 3, 4, 5, tzinfo=pytz.utc), "%d/%m/%Y")
        == "02/01/1234"
    )
...
def test_b_upper():
    assert (
        safe_strftime(datetime(1234, 1, 2, 3, 4, 5, tzinfo=pytz.utc), "%B") == "January"
    )
    assert (
        safe_strftime(datetime(1234, 2, 2, 3, 4, 5, tzinfo=pytz.utc), "%B")
        == "February"
    )
    assert (
        safe_strftime(datetime(1234, 3, 2, 3, 4, 5, tzinfo=pytz.utc), "%B") == "March"
    )
    assert (
        safe_strftime(datetime(1234, 4, 2, 3, 4, 5, tzinfo=pytz.utc), "%B") == "April"
    )
    ...
    assert (
        safe_strftime(datetime(1234, 8, 2, 3, 4, 5, tzinfo=pytz.utc), "%B") == "August"
    )
    assert (
        safe_strftime(datetime(1234, 9, 2, 3, 4, 5, tzinfo=pytz.utc), "%B")
        == "September"
    )
    assert (
        safe_strftime(datetime(1234, 10, 2, 3, 4, 5, tzinfo=pytz.utc), "%B")
        == "October"
    )
    assert (
        safe_strftime(datetime(1234, 11, 2, 3, 4, 5, tzinfo=pytz.utc), "%B")
        == "November"
    )
    assert (
        safe_strftime(datetime(1234, 12, 2, 3, 4, 5, tzinfo=pytz.utc), "%B")
        == "December"
    )
...
def test_f_lower():
    assert (
        safe_strftime(datetime(1234, 1, 2, 3, 4, 5, 123, tzinfo=pytz.utc), "%f")
        == "000123"
    )
...
def test_percent():
    assert (
        safe_strftime(datetime(1234, 1, 2, 3, 4, 5, tzinfo=pytz.utc), "100%%") == "100%"
    )
