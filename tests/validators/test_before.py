...
from cornflake.validators import before


def test_date_datetime():
    value = before(date(2015, 1, 1))(
...
def test_less_than():
    before(datetime(2015, 1, 1, 0, 0, 0, tzinfo=pytz.utc))(
        datetime(2014, 12, 31, 23, 59, 59, tzinfo=pytz.utc)
    )
...
def test_dt_format():
    with pytest.raises(ValidationError) as e:
        before(datetime(2015, 1, 1, 0, 0, 0, tzinfo=pytz.utc), dt_format="%Y-%m-%d")(
